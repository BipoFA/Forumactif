    ...
    $(function () {
      ...
      $form.on('submit', function (event) {
        ...
        if ($textarea.length === 0) {
          ...
          alert('[Erro Codigo 001 | Form Functions] Tente novamente!');
          return false;
        }
  
        $textarea.val($textarea.val().replace(/\{\{fields(\d+)\}\}/gi, function (text, match) {
          return $('#fields' + match).val();
        }));
        ...
        $title.val($title.val().replace(/\{\{fields(\d+)\}\}/gi, function (text, match) {
          return $('#fields' + match).val();
        }));
      });
    });
